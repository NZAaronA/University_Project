...
function addComment() {
    const url = 'https://cws.auckland.ac.nz/ako/api/Comment';
...
function displayLog(data, colour) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        htmlString += `<svg width="50" height="25" style="text-anchor: middle; display:inline">
        ...
        for (let j = 0; j < quotient; j++) {
            htmlString += '<svg width="25" height="25" ><use href="#koru"></svg>';
        }

        clipPathsString += `<clipPath id=cutout${i}><rect width="${2.5 * remainder}" height="25" /></clipPath>`;
        htmlString += `<svg width="25" height="25" clip-path="url(#cutout${i})"><use href="#koru"></svg><br>`;
    }
